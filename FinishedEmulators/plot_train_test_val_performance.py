...
data_       = np.genfromtxt('../Pipeline/neural_network_emulator/data/val.csv', delimiter=',')[1:]
data_train_ = np.genfromtxt('../Pipeline/neural_network_emulator/data/train.csv', delimiter=',')[1:]
data_test_  = np.genfromtxt('../Pipeline/neural_network_emulator/data/test.csv', delimiter=',')[1:]
...
# For each version of the emulator we make a figure
print("Version = ", emulator_version)
...
for i, data_i in enumerate(data_train): 
    ...
    # Call emulator
    _emulator    = PofkBoostEmulator(path = emulator_folder, version = emulator_version)
    emuresult    = _emulator(params_batch)
    
    # Make plot
    axs[0].semilogx(k, emuresult, color=colors[1], linewidth=0.7, alpha=0.5)
    axs[1].semilogx(k, emuresult/boost - 1.0, color=colors[0], linewidth=0.7, alpha=0.5)
...
for i, data_i in enumerate(data): 
    ...
    # Make plot
    axs[3].semilogx(k, emuresult, color=colors[1], linewidth=0.7, alpha=0.5)
    axs[4].semilogx(k, emuresult/boost - 1.0, color=colors[0], linewidth=0.7, alpha=0.5)
...
for i, data_i in enumerate(data_test): 
    ...
    # Make plot
    axs[6].semilogx(k, emuresult, color=colors[1], linewidth=0.7, alpha=0.5)
    axs[7].semilogx(k, emuresult/boost - 1.0, color=colors[0], linewidth=0.7, alpha=0.5)
...
print("Saving plot ", figpath)
plt.show()
fig.savefig(figpath, format='pdf', bbox_inches='tight')
print(f'save plot to {figpath}')
plt.close(fig)
